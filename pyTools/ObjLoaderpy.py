...
class ObjLoader:
    ...
    @staticmethod
    def load_model(file, bahn_parameter=False, sorted=True):
        ...
        if bahn_parameter:
            with open(file, 'rb') as f:
                ...
                while line:
                    values = line.split()
                    if values[0] == 'v':
                        ve.append(round(float(values[1]), 5))
                        ve.append(round(float(values[2]), 5))
                        ve.append(round(float(values[3]), 5))
                        vert_list.append(ve)
                        ve = []
                        pass
                    line = f.readline()
...
if __name__ == '__main__':
    bahn = ObjLoader.load_model("Spline.obj",bahn_parameter = True)
